defmodule Skygarden.EventBulkMessagesTest do
  ...
  describe "event_bulk_messages" do
    ...
    @invalid_attrs %{
      ...
      sent: nil,
      send_at: nil,
      number_of_users_sent_to: nil,
      send_via: nil,
      send_to_a_specific_ticket_group: nil
    }
    ...
    test "get_event_bulk_message!/1 returns the event_bulk_message with given id" do
      ...
      assert EventBulkMessages.get_event_bulk_message!(event_bulk_message.id) ==
               event_bulk_message
    end
    ...
    test "create_event_bulk_message/1 with valid data creates a event_bulk_message" do
      ...
      assert {:ok, %EventBulkMessage{} = event_bulk_message} =
               EventBulkMessages.create_event_bulk_message(valid_attrs)
    ...
    test "create_event_bulk_message/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} =
               EventBulkMessages.create_event_bulk_message(@invalid_attrs)
    end
    ...
    test "update_event_bulk_message/2 with valid data updates the event_bulk_message" do
      ...
      update_attrs = %{
        ...
        sent: false,
        send_at: ~N[2025-04-22 16:23:00],
        number_of_users_sent_to: 43,
        send_via: "some updated send_via",
        send_to_a_specific_ticket_group: false
      }

      assert {:ok, %EventBulkMessage{} = event_bulk_message} =
               EventBulkMessages.update_event_bulk_message(event_bulk_message, update_attrs)
    ...
    test "update_event_bulk_message/2 with invalid data returns error changeset" do
      ...
      assert {:error, %Ecto.Changeset{}} =
               EventBulkMessages.update_event_bulk_message(event_bulk_message, @invalid_attrs)

      assert event_bulk_message ==
               EventBulkMessages.get_event_bulk_message!(event_bulk_message.id)
    end

    test "delete_event_bulk_message/1 deletes the event_bulk_message" do
      event_bulk_message = event_bulk_message_fixture()

      assert {:ok, %EventBulkMessage{}} =
               EventBulkMessages.delete_event_bulk_message(event_bulk_message)
