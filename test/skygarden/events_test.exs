defmodule Skygarden.EventsTest do
  ...
  describe "events" do
    ...
    test "create_event/1 with valid data creates a event" do
      valid_attrs = %{active: true, name: "some name", date: ~D[2025-04-29], description: "some description", location: "some location", poster_image: "some poster_image", start_time: ~T[14:00:00], end_time: ~T[14:00:00], remind_time: ~T[14:00:00], slug: "some slug", remind_after: 42}

      assert {:ok, %Event{} = event} = Events.create_event(valid_attrs)
      assert event.active == true
      assert event.name == "some name"
      assert event.date == ~D[2025-04-29]
      assert event.description == "some description"
      assert event.location == "some location"
      assert event.poster_image == "some poster_image"
      assert event.start_time == ~T[14:00:00]
    ...
    test "update_event/2 with valid data updates the event" do
      event = event_fixture()
      update_attrs = %{active: false, name: "some updated name", date: ~D[2025-04-30], description: "some updated description", location: "some updated location", poster_image: "some updated poster_image", start_time: ~T[15:01:01], end_time: ~T[15:01:01], remind_time: ~T[15:01:01], slug: "some updated slug", remind_after: 43}

      assert {:ok, %Event{} = event} = Events.update_event(event, update_attrs)
      assert event.active == false
      assert event.name == "some updated name"
      assert event.date == ~D[2025-04-30]
      assert event.description == "some updated description"
      assert event.location == "some updated location"
      assert event.poster_image == "some updated poster_image"
      assert event.start_time == ~T[15:01:01]
